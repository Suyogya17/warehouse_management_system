...
function OfferProductCard({ variants, canManage, canOrder, viewer, onEdit, onRemove, onAddToCart, onProductInterest, cartProductIds }) {
  ...
  const expired = isExpiredOffer(selected);
  ...
  const customerOfferPrice =
    Number.isFinite(backendOfferPrice) && backendOfferPrice > 0
      ...
      : Number(customerBasePrice) > 0
        ...
        : null;
  return (
    <article className={`group flex flex-col overflow-hidden rounded-2xl border bg-white transition-all duration-300 hover:shadow-xl ${active ? "border-amber-300" : expired ? "border-red-300" : "border-slate-200"}`}>
      <div className="relative aspect-[5/3] overflow-hidden bg-slate-100">
        ...
        <span className={`absolute left-3 top-3 inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-bold text-white ${active ? "bg-amber-500" : expired ? "bg-red-600" : "bg-slate-500"}`}><Tag size={13} />{active ? "ON OFFER" : expired ? "EXPIRED" : "NOT ON OFFER"}</span>
      ...
      <div className="flex flex-1 flex-col gap-2 p-3">
        ...
        {active || expired ? (
          <div className={`rounded-xl p-2 ${expired ? "bg-red-50" : "bg-amber-50"}`}>
            <p className={`text-xs font-semibold uppercase ${expired ? "text-red-700" : "text-amber-700"}`}>{expired ? "Expired offer" : selected.offer_label || "Special offer"}</p>
            {selected.offer_ends_at && <p className="mt-1 text-xs text-slate-500">{expired ? "Ended" : "Ends"} {new Date(selected.offer_ends_at).toLocaleString()}</p>}
        ...
        {canManage && <div className="mt-auto flex gap-2"><Button type="button" onClick={() => onEdit(selected)}>{active ? "Edit offer" : expired ? "Extend offer" : "Add offer"}</Button>{(active || expired) && <Button type="button" variant="secondary" onClick={() => onRemove(selected)}>Remove</Button>}</div>}
        {!canManage && canOrder && (() => {
