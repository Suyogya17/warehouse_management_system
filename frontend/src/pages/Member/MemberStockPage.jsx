...
export default function MemberStockPage() {
  ...
  const loadRawMaterials = useCallback(async () => {
    ...
    setRawMaterialAvailability(result.data || []);
  }, [token]);
  ...
  useEffect(() => {
    ...
    loadRawMaterials().catch((error) => {
      showToast({
        tone: "error",
        title: "Raw material stock failed to load",
        message: error.message || "Could not load raw material stock availability.",
      });
    });
  }, [load, loadRawMaterials, showToast]);
  ...
  const filteredRawMaterialAvailability = useMemo(() => {
      ...
      <SectionCard title="Raw Material Availability" icon="stock">
        <div className="p-4">
          ...
          <DataTable
            ...
            rows={filteredRawMaterialAvailability}
            emptyTitle="No stock found"
            emptyDescription="Try a different search or stock filter."
          />
        </div>
