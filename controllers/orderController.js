  ...
  try {
    ...
    return res.json({
      ...
      data: products.rows.map((p) => {
        ...
        const available_qty = Math.max(0, display_stock - reserved_qty);

        return {
          ...p,
          physical_stock,            // raw warehouse count
          display_stock,             // user-visible cap, never above 450
          reserved_qty,
          available_qty,             // what users can still order
        };
      }),
    });
  } catch (err) {
...
//       // Check physical stock accounts for all OTHER active reservations too
...
//         // Remove this order's own qty to get competing reservations only
...
//       // Deduct from physical quantity only — display_quantity is no longer used
//       for (const item of itemsRes.rows) {
//         const { clause: fgClause, params: fgParams } = buildInClause([item.finished_good_id]);
//         await client.query(
